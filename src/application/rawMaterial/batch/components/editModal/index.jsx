...
import { updateSchema } from "application/rawMaterial/batch/schema";
import { updateRawMaterialBatch } from "application/rawMaterial/batch/service";
...
export default function EditRawMaterialBatchModal({
  showModal,
  onShowModalChange,
  batch,
  onEdit,
  ...
  const handleSubmit = async (e) => {
    ...
    const data = {
      warehouseId: warehouse ? warehouse?.id : batch?.warehouse.id,
    ...
    updateRawMaterialBatch(batch?.id, data)
      .then((response) => {
        ...
        toast.success("Lote de materia prima actualizado");
        onEdit();
        handleClose();
      })
      .catch((err) => {
          ...
          <div className="flex justify-between items-start p-5 rounded-t border-b">
            <div>
              <h3 className="font-medium text-lg">
                Editar lote de materia prima
              </h3>
              <p className="pt-2 text-sm text-gray-400">
                Esta acción editará la información sobre el lote de materia
                prima ya existente, esta acción no se puede revertir.
              </p>
            </div>
            <button
              onClick={handleClose}
                ...
                <div className="col-span-6 sm:col-span-3">
                  ...
                  <div className="flex">
                    <input
                      ...
                      value={capitalize(batch?.rawMaterial.name)}
                    />
                  </div>
                </div>
                <div className="col-span-6 sm:col-span-3">
                  ...
                  <AsyncSelect
                    ...
                    defaultOptions
                    placeholder={capitalize(batch?.warehouse.name)}
                    ...
                    {batch?.stock > 0 ? (
                      <div className="flex">
                        ...
                        <input
                          disabled
                          className="appearance-none block w-full text-gray-900 focus:ring-yellow-100 focus:border-beereign_yellow shadow-sm border border-gray-300 rounded py-3 px-4 mb-3 leading-tight focus:outline-none"
                          value={batch?.rawMaterial.measurement}
                        />
                      </div>
                    ) : (
                      <div className="flex">
